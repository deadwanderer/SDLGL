  ...
  private:
    ...
    void processNode(aiNode *node, const aiScene *scene) {
        ...
        for (unsigned int i = 0; i < node->mNumMeshes; i++) {
            ...
            // the scen contains all the data, node is just to keep things organized (like relations between nodes).
            aiMesh *mesh = scene->mMeshes[node->mMeshes[i]];
            meshes.push_back(processMesh(mesh, scene));
        }

        // after we've processed all the meshes (if any), we then recursively process each of the child nodes.
    ...
    vector<Texture> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName) {
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
            ...
            if (!skip) {
                // If texture hasn't already been loaded, load it.
                Texture texture;
